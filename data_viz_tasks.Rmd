...
```{r t13}
gross<- ggplot(data = movies, aes(x= production_budget, y=domestic_gross))+
  geom_point()
...
#Create a subplot with 2 columns based on genre:
gross+ facet_wrap(facets = ~genre  , ncol =  2 ) #<<

```

![](`r knitr::fig_chunk('t13', 'png')`)

---

## Your turn 14

Create a subplot based on genre and mapp_rating.

```{r t14}
gross<- ggplot(data = movies, aes(x= production_budget, y=domestic_gross))+
  geom_point()

#gross is the scatter plot between production budget and domestic gross.

#Create a subplot based on genre and mpaa_rating.


gross+facet_grid(genre~ mpaa_rating     )
```

![](`r knitr::fig_chunk('t14', 'png')`)
